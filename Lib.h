...
void LED_SHIFT_CTRL();
real_number num_conv (float number);
void TEMP_CHECK(float temperature);
unsigned char PB_CHECK();
			...
			_delay_ms(500); break;		}		else if (push_button == 1 || push_button == 2)
		{
			...
			while(1)
			{
				...
				else
				{
					...
					temp = num_conv(result);
					LCD4_CUR_GOTO(push_button,6);
					LCD4_OUT_DEC(temp.integer_part,3);					LCD4_OUT_STR(".");					LCD4_OUT_DEC(temp.decimal_part,2);
					_delay_us(240);
				}
			}
		}	}}void ADC_2_LCD(){	ADC_STA_CONVERT();	// Bắt đầu nhận adc	float temp = ADC/4;		real_number num = num_conv(temp);	// In nhiệt độ đo được ra màn hình	LCD4_CUR_GOTO(2,6);	LCD4_OUT_DEC(num.integer_part,3);	LCD4_OUT_STR(".");	LCD4_OUT_DEC(num.decimal_part,2);	_delay_ms(1000);
