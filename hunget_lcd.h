...
#define lcd4_control_PORT	PORTA
#define lcd4_control_DDR 	DDRA
...
#define lcd4_data_PORT		PORTB
#define lcd4_data_DDR		DDRB
...
void LCD4_INIT(unsigned char lcd4_cur_underline, unsigned char lcd4_cur_blink)
{
	...
	_delay_ms(40);						// Cho it nhat 40 ms cho LCD on dinh
	...
	for(lcd4_i=0;lcd4_i<3;lcd4_i++)
	{
		...
		lcd4_control_PORT &= ~(1<<E);	
		_delay_us(200);						// Tre it nhat 160 us de hoan thanh lenh
	}
	...
	_delay_us(4100);							// Tre it nhat 4.1 ms de hoan thanh lenh

	LCD4_OUT_CMD(lcd4_cur_mode);			// Bat hien thi, va khoi tao con tro.
	_delay_us(4100);							// Tre mot luc
...
void LCD4_OUT_CMD(unsigned char lcd4_cmd)
{
	...
	lcd4_control_PORT &= ~(1<<E);
	_delay_us(100);							// Tre mot luc. 100 us
